...
export default function EditarTimePage() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      anoFundacao: String(time.anoFundacao),
      telefone: "",
    });
    if (time.logoUrl) setPreview(time.logoUrl);
  }, [id]);
  ...
  function handleSubmit() {
    ...
    console.log("Atualizar time:", { id, ...form, preview });
    router.push("/times");
  }
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-[#F1F3F7] px-4 py-4 md:px-5 md:py-5 lg:px-6 lg:py-6">
      ...
      <PageHeader
        title="Editar Time"
        description="Atualize os dados do time"
      />
      ...
      <div className="w-full max-w-[700px] mx-auto">
        <div className="bg-white rounded-[12px] border border-[#E5E7EB] shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          ...
          {/* Footer */}
          <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-[#F1F5F9] bg-[#FAFAFA] rounded-b-[12px]">
            <button
              onClick={() => router.push("/times")}
